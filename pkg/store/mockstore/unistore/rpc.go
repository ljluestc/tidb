...
func (c *RPCClient) SendRequest(ctx context.Context, addr string, req *tikvrpc.Request, timeout time.Duration) (*tikvrpc.Response, error) {
	...
	if val, _err_ := failpoint.Eval(_curpkg_("epochNotMatch")); _err_ == nil {
		if val.(bool) {
	...
	if val, _err_ := failpoint.Eval(_curpkg_("unistoreRPCDeadlineExceeded")); _err_ == nil {
		if val.(bool) && timeout < time.Second {
			return tikvrpc.GenRegionErrorResp(req, &errorpb.Error{Message: "Deadline is exceeded"})
		}
	}
	if val, _err_ := failpoint.Eval(_curpkg_("unistoreRPCSlowByInjestSleep")); _err_ == nil {
		time.Sleep(time.Duration(val.(int) * int(time.Millisecond)))
		return tikvrpc.GenRegionErrorResp(req, &errorpb.Error{Message: "Deadline is exceeded"})
	}
	if val, _err_ := failpoint.Eval(_curpkg_("unistoreRPCSlowCop")); _err_ == nil {
	...
	case tikvrpc.CmdMPPConn:
		...
		if val, _err_ := failpoint.Eval(_curpkg_("MppVersionError")); _err_ == nil {
